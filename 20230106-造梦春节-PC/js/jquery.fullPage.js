	...
	$.fn.fullpage = function (options) {
		...
		*/
		...
		function scrollPage(element, callback, isMovementUp) {
			...
			var mnli = $('#menu li').eq(v.sectionIndex)
			if (!mnli.hasClass('dis')) {
				mnli.addClass('active').siblings().removeClass('active')
			}
		...
		function performMovement(v) {
			...
			if (options.css3 && options.autoScrolling && !options.scrollBar) {
				// 修改--footer的索引，需要配合样式调整：footer需要设置对应所需高度
				if (v.sectionIndex == options.footerIndex) { //判断当滚动到尾屏如何做
					footer_a = $('.section-foot').height();
					footer_h = $('.footerArea').outerHeight();
		...
		function getScrollSettings(v) {
			...
			if (options.autoScrolling && !options.scrollBar) {
				...
				{
					footer_a = $('.section-foot').height();
					footer_h = $('.footerArea').height();
					scroll.options = {
						'top': -(v.dtop - footer_a + footer_h)
					};
				} else {
					scroll.options = {
						'top': -v.dtop
		...
		function resizeMe(displayHeight, displayWidth) {
			...
			$('.commentArea').height(displayHeight - 300)
